...
curl -X POST --header "Content-Type:multipart/form-data" --header "Authorization:Token $DOJOKEY" -F "engagement=${EGID}" -F "scan_type=MobSF Scan" -F 'file=@./output/report.json' --url "${DOJOURL}/api/v2/import-scan/"
